...
if not os.path.exists(outdir):
    os.makedirs(outdir)
Infile1 = "rpc_2017_2018_HV_Imon.xlsx"
Infile2 = "Copy_of_RPC_Chamber_HV_Map.xlsx"
...
#      print('args.w_r_name: %s rpc_names[sheet2_idx]: %s'%(args.w_r_name,rpc_names[sheet2_idx] ))
  ...
  if len(voltage1)==0 and len(voltage2)==0 and len(voltage3)==0 and len(voltage4)==0 :
    continue
  if len(voltage1)!=0:
    plt.scatter(voltage1, current1,color = 'red', label = "Mar-18",marker='o')
  if len(voltage2)!=0:
    plt.scatter(voltage2, current2,color="blue", label = "Apr-17", marker='o')
  if len(voltage3)!=0:
    plt.scatter(voltage3, current3,color="green", label = "Jul-17",marker='o')
  if len(voltage4)!=0:
    plt.scatter(voltage4, current4,color="magenta", label = "Oct-17",marker='o')
...
##################################################
if len(vapp_avg1)!=0:
  plt_wheel.scatter([*map(mean, zip(*vapp_avg1))], [*map(mean, zip(*imon_avg1))], color = 'red', label = "Mar-18",marker='o')
if len(vapp_avg2)!=0:
  plt_wheel.scatter([*map(mean, zip(*vapp_avg2))], [*map(mean, zip(*imon_avg2))], color = 'blue', label = "Apr-17",marker='o')
if len(vapp_avg3)!=0:
  plt_wheel.scatter([*map(mean, zip(*vapp_avg3))], [*map(mean, zip(*imon_avg3))], color = 'green', label = "Jul-17",marker='o')
if len(vapp_avg4)!=0:
  plt_wheel.scatter([*map(mean, zip(*vapp_avg4))], [*map(mean, zip(*imon_avg4))], color = 'magenta', label = "Oct-17",marker='o')
...
if len(v_fit_1)!=0:
  ...
  plt_fit.scatter([*map(mean, zip(*vapp_avg1))], [*map(mean, zip(*imon_avg1))], color = 'red', label = "Mar-18",marker='o')
...
avg_delta_6kV_xaxis = ['Apr17-Mar18', 'Jul17-Apr17', 'Oct17-Jul17']
if len(i_fit_final_2)==0:
  avg_delta_95kV_xaxis = ['Mar18','Jul17','Oct17' ]
  ...
  avg_delta_95kV_xaxis = ['Mar18','Apr17','Jul17','Oct17' ]
