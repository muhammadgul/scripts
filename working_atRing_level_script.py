...
if not os.path.exists(outdir):
    os.makedirs(outdir)
Infile1 = "rpc_2016_2017_HV_Imon.xlsx"
...
#      print('args.w_r_name: %s rpc_names[sheet2_idx]: %s'%(args.w_r_name,rpc_names[sheet2_idx] ))
  ...
  if len(voltage1)==0 and len(voltage2)==0 and len(voltage3)==0 and len(voltage4)==0 :
    continue
  if len(voltage1)!=0:
    plt.scatter(voltage1, current1,color = 'red', label = "Sep-16",marker='o')
  if len(voltage2)!=0:
...
##################################################
if len(vapp_avg1)!=0:
  plt_wheel.scatter([*map(mean, zip(*vapp_avg1))], [*map(mean, zip(*imon_avg1))], color = 'red', label = "Sep-16",marker='o')
if len(vapp_avg2)!=0:
...
'''
plt.scatter(v1, i1,color="red", label = "Sep-16")
